...
class Compilation {
	...
	constructor(compiler) {
		...
		this.factorizeQueue = new AsyncQueue({
			name: "factorize",
			parallelism: options.parallelism || 100, // 最大并行树
			processor: this._factorizeModule.bind(this) // 处理模块方式
		});
		...
		//READ 依赖工厂 key为依赖的构造函数, value为一个工厂函数
		//可以通过构造函数找到对应的工厂函数创建类实例
		...
		//READ 调试hooks lifecycle
		// Object.keys(this.hooks).forEach(hook => {
		// 	this.hooks[hook].intercept({
		// 		call: (source, target, routesList) => {
		// 			console.log(`开始调用${hook}钩子`);
		// 		},
		// 		register: tapInfo => {
		// 			console.log(`${tapInfo.name}正在调用register`);
		// 			return tapInfo;
		// 		}
		// 	});
		// });
	...
	_factorizeModule(
		...
		//READ 调用对应的工厂函数的create方法
		//一般来说factory是NormalModuleFactory工厂函数
		factory.create(
			{
				contextInfo: {
	...
	addModuleChain(context, dependency, callback) {
		...
		const Dep = /** @type {DepConstructor} */ (dependency.constructor); // 获取入口对象的构造函数
		const moduleFactory = this.dependencyFactories.get(Dep); // 去依赖工程找这个入口构造函数
		if (!moduleFactory) {
			return callback(
				new WebpackError(
	...
	addEntry(context, entry, optionsOrName, callback) {
		...
		const { name } = options;
		let entryData = this.entries.get(name); //READ 获取已有的入口数据
		//READ 如果没有则初始化入口数据
		if (entryData === undefined) {
			entryData = {
				dependencies: [entry], // EntryDependency
				options: {
					name: undefined,
					...options
				}
			};
			this.entries.set(name, entryData); //READ 存入入口数据name为main
		} else {
		...
		//READ lib/ProgressPlugin.js entriesCount++
		this.hooks.addEntry.call(entry, options); //READ 处理添加入口，这里会更新它的entriesCount
		// 添加模块链
