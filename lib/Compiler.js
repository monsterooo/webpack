...
class Compiler {
	...
	constructor(context) {
		...
		this.hooks = Object.freeze({
			...
			/** @type {AsyncSeriesHook<[Compiler]>} */
			//READ 在Compiler开始读取`records`之前
			run: new AsyncSeriesHook(["compiler"]),
			/** @type {AsyncSeriesHook<[Compilation]>} */
			//READ 将assets输出到目录前执行
	...
	newCompilation(params) {
		//READ 根据compiler对象创建一个Compilation对象
		const compilation = this.createCompilation();
		compilation.name = this.name;
		compilation.records = this.records;
		this.hooks.thisCompilation.call(compilation, params); //READ 在初始化compilation时执行并且在`compilation`时间之前执行
		this.hooks.compilation.call(compilation, params); //READ 运行plugin在创建compilation之后
		return compilation;
	}
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			//READ 调用钩子开始编译 => hooks.make.tapAsync // lib/EntryPlugin.js
