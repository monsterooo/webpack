...
class WebpackOptionsApply extends OptionsApply {
	 ...
	 */
	//READ 一些钩子的初始化将会在这里进行，还是比较重要
	process(options, compiler) {
		compiler.outputPath = options.output.path; // 输出路径
		...
		//READ 调用初始化编译入口的钩子就是在这里被初始化
		//READ 这里主要是给compiler.hooks.entryOption挂上`EntryOptionPlugin`插件
		//处理并添加入口
		new EntryOptionPlugin().apply(compiler);
		//READ 紧接着调用上面的EntryOptionPlugin钩子，进行入口初始化处理
