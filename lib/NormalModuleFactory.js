...
class NormalModuleFactory extends ModuleFactory {
	constructor({ context, fs, resolverFactory, options }) {
		...
		this.hooks.factorize.tapAsync(
			...
			(resolveData, callback) => {
				//READ 触发hooks.resolve钩子去解析loader以及rules然后是加载资源文件
		...
		this.hooks.resolve.tapAsync(
			...
			(data, callback) => {
				...
				const loaderResolver = this.getResolver("loader"); //READ 获取一个resolve
				const normalResolver = this.getResolver("normal", resolveOptions); //READ 获取一个resolve
	...
	create(data, callback) {
		const dependencies = /** @type {ModuleDependency[]} */ (data.dependencies); // [EntryDependency]
		if (this.unsafeCache) {
		...
		const request = dependency.request; // entry
		...
		this.hooks.beforeResolve.callAsync(resolveData, (err, result) => {
			...
			// Ignored
			//READA 如果这里调用IgnorePlugin返回false则代表忽略这个文件
			if (result === false) {
	...
	resolveRequestArray( //READ 对多个inline loader进行解析
		contextInfo,
		context,
		array,
		resolver,
